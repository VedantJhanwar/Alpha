...
def fetch_live_info(ticker: str) -> dict:
    """
    Fetch live fundamental data and metadata for a given ticker.
    This provides 'screener-like' context (NAV, Market Cap, P/E, etc.)
    ...
    try:
        ...
        logger.warning("Failed to fetch live info for %s: %s", ticker, str(e))
        return {}
...
def search_ticker(query: str) -> list[dict]:
    """
    Search Yahoo Finance for a company name or ticker symbol.
    Returns a list of dictionaries with matching symbols and names.
    """
    ...
    try:
        ...
        logger.warning("Failed to search Yahoo Finance for '%s': %s", query, str(e))
        return []
